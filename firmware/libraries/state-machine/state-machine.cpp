...
/* Platform Navigation Mode */
State modePlatformNavigation( State currentState,
                              long stateChangeTimestamp,
                              boolean btnState[BTN_COUNT],
                              boolean ledState[LED_COUNT],
                              boolean flags[FLAG_COUNT],
                            ...
                            StateEstimator *stateEstimator)
{
  switch (mode)
  {
    ...
    case m_platform_navigation: return modePlatformNavigation(currentState, stateChangeTimestamp, btnState, ledState, flags, stateEstimator);
...
// State BUTTON path: -> following -> following slope -> wander -> returning1 -> drop -> returning2
State modePlatformNavigation( State currentState,
                              long stateChangeTimestamp,
                              boolean btnState[BTN_COUNT],
                              boolean ledState[LED_COUNT],
                              ...
                              StateEstimator *stateEstimator)
{
  ...
  else if (currentState == s_following)
  {
    ...
    else if (flags[FLAG_ON_PLATFORM] && flags[FLAG_FOLLOWING_CORNER_DETECTED] && stateTransitionAllowed)
    {
      stateEstimator->correctPos(CORRECT_RETURNING_SLOPE_X,CORRECT_RETURNING_SLOPE_Y);

      flags[FLAG_FOLLOWING_CORNER_DETECTED] = 0;

      return s_following_slope_drop;
    }
    // Exit following state and enter turn state to follow corner
  ...
  {
    if (flags[FLAG_TURN_FINISHED] && flags[FLAG_ON_PLATFORM] && flags[FLAG_SWALLOWED_BOTTLE] && stateTransitionAllowed)
    {
      flags[FLAG_TURN_FINISHED] = 0;

      return s_following;
    }
    else if (flags[FLAG_TURN_FINISHED] && flags[FLAG_ON_PLATFORM] && stateTransitionAllowed)
    {
      flags[FLAG_TURN_FINISHED] = 0;
  ...
  {
    if ((flags[FLAG_ON_PLATFORM] && flags[FLAG_RETURNED] && flags[FLAG_SWALLOWED_BOTTLE] && stateTransitionAllowed) || nextState)
    {
      flags[FLAG_RETURNED] = 0;
    ...
    else if ((flags[FLAG_RETURNED] && flags[FLAG_SWALLOWED_BOTTLE] && stateTransitionAllowed) || nextState)
    {
      flags[FLAG_RETURNED] = 0;
  ...
  /**
   * Current state "s_following_slope_drop"
   */
  else if (currentState == s_following_slope_drop)
  {
    if ((!flags[FLAG_ON_PLATFORM] && stateTransitionAllowed) || nextState)
    {
      stateEstimator->correctPos(CORRECT_FOLLOWING_DROP_SLOPE_X,CORRECT_FOLLOWING_DROP_SLOPE_Y);
